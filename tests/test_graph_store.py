...
def test_insert_rows_returns_count(graph_db):
    conn, movie, src, rev = graph_db
    n = graph_store.insert_rows(
        conn,
        source=src,
        spec_revision=rev,
...
def test_insert_rows_creates_current_binding(graph_db):
    conn, movie, src, rev = graph_db
...
def test_insert_rows_binding_has_null_valid_to(graph_db):
    conn, movie, src, rev = graph_db
    graph_store.insert_rows(
        conn, source=src, spec_revision=rev,
...
def test_canonical_id_exists_returns_false_for_unknown(graph_db):
    conn, movie, src, rev = graph_db
    assert not graph_store.canonical_id_exists(conn, cls=movie, canonical_id="tt_not_here")
...
def test_get_canonical_contributions_returns_empty_for_unknown(graph_db):
    conn, movie, src, rev = graph_db
    contribs = graph_store.get_canonical_contributions(conn, cls=movie, canonical_id="unknown_id")
    assert contribs == []
...
def test_merge_closes_source_binding_and_opens_new(graph_db):
    ...
    assert not graph_store.canonical_id_exists(conn, cls=movie, canonical_id="tt_merge")
    # The row is now bound to tt_keep
    assert graph_store.canonical_id_exists(conn, cls=movie, canonical_id="tt_keep")
...
def test_partial_unique_index_holds_after_merge(graph_db):
