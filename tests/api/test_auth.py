...
async def test_find_by_key_hash_returns_user(auth_db):
    _, raw_key = await create_user(auth_db, username="dave")
    found = await find_by_key_hash(auth_db, hash_key(raw_key))
    assert found is not None
    assert found.username == "dave"


async def test_find_by_key_hash_returns_none_for_unknown(auth_db):
    result = await find_by_key_hash(auth_db, "a" * 64)
    assert result is None


async def test_get_user_returns_user(auth_db):
    await create_user(auth_db, username="eve")
    found = await get_user(auth_db, "eve")
    assert found is not None
    assert found.username == "eve"


async def test_get_user_returns_none_for_unknown(auth_db):
    assert await get_user(auth_db, "no_such_user") is None


async def test_create_user_records_created_by(auth_db):
    await create_user(auth_db, username="admin_user", is_admin=True)
    await create_user(auth_db, username="regular", created_by="admin_user")
    regular = await get_user(auth_db, "regular")
    assert regular.created_by == "admin_user"


async def test_create_admin_user_is_admin(auth_db):
    user, _ = await create_user(auth_db, username="superuser", is_admin=True)
    assert user.is_admin is True


async def test_rotate_key_returns_new_raw_key(auth_db):
    _, old_raw = await create_user(auth_db, username="rotate_me")
    new_raw = await rotate_key(auth_db, username="rotate_me")
    assert new_raw != old_raw
    # New key works
    found = await find_by_key_hash(auth_db, hash_key(new_raw))
    assert found is not None
    # Old key no longer works
    assert await find_by_key_hash(auth_db, hash_key(old_raw)) is None


async def test_delete_user_removes_row(auth_db):
    await create_user(auth_db, username="to_delete")
    deleted = await delete_user(auth_db, username="to_delete")
    assert deleted is True
    assert await get_user(auth_db, "to_delete") is None


async def test_delete_unknown_user_returns_false(auth_db):
    assert await delete_user(auth_db, username="ghost") is False
...
async def test_bootstrap_admin_if_empty_inserts_admin_when_table_empty(auth_db):
    ...
    admin = await get_user(auth_db, "admin")
    assert admin is not None
    assert admin.is_admin is True


async def test_bootstrap_admin_if_empty_is_idempotent(auth_db):
    await bootstrap_admin_if_empty(auth_db, "key1")
    # Second call with any key when a user exists should return False
    result = await bootstrap_admin_if_empty(auth_db, "key2")
    assert result is False


async def test_bootstrap_admin_key_works_for_auth(auth_db):
    raw_key = "test_bootstrap_key_abc123"
    await bootstrap_admin_if_empty(auth_db, raw_key)
    found = await find_by_key_hash(auth_db, hash_key(raw_key))
    assert found is not None
    assert found.username == "admin"


async def test_bootstrap_admin_does_not_insert_when_users_exist(auth_db):
    await create_user(auth_db, username="existing")
    inserted = await bootstrap_admin_if_empty(auth_db, "any_key")
    assert not inserted
    # No "admin" row was created
    assert await get_user(auth_db, "admin") is None
...
async def test_correct_token_returns_200_when_auth_enforced(auth_db):
