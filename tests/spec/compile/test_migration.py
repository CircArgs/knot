...
def test_apply_changes_creates_source_table(clean_db):
    spec = _minimal_spec()
    changes = diff_specs(None, spec)
    apply_changes(clean_db, changes)
    assert _table_exists(clean_db, "movie")


def test_apply_changes_creates_bindings_table(clean_db):
    spec = _minimal_spec()
    changes = diff_specs(None, spec)
    apply_changes(clean_db, changes)
    assert _table_exists(clean_db, "movie_bindings")


def test_apply_changes_creates_partial_unique_index_on_bindings(clean_db):
    spec = _minimal_spec()
    changes = diff_specs(None, spec)
    apply_changes(clean_db, changes)
    # The partial UNIQUE index enforcing one current binding per knot_row_id
    assert _index_exists(clean_db, "movie_bindings_one_current_per_row")


def test_apply_changes_is_idempotent(clean_db):
    spec = _minimal_spec()
    changes = diff_specs(None, spec)
    apply_changes(clean_db, changes)
    apply_changes(clean_db, changes)  # second run should not raise
    assert _table_exists(clean_db, "movie")


def test_apply_changes_add_slot_creates_column(clean_db):
    st = _str_type()
    id_slot = Slot(name="imdb_id", range=st, identifier=True)
    movie = OntologyClass(name="Movie", slots=[id_slot])
    v1 = Spec(id="t", version="1.0.0", types=[st], slots=[id_slot], classes=[movie])
    apply_changes(clean_db, diff_specs(None, v1))
    ...
    apply_changes(clean_db, diff_specs(v1, v2))
    assert _column_exists(clean_db, "movie", "title")
...
def test_publish_blocks_destructive_slot_drop_without_flag(clean_db):
    st = _str_type()
    id_slot = Slot(name="imdb_id", range=st, identifier=True, required=True)
    title = Slot(name="title", range=st)
    ...
    rev1 = create_draft(clean_db)
    update_draft(clean_db, rev1, spec_v1)
    publish_draft(clean_db, rev1)
    ...
    rev2 = create_draft(clean_db)
    update_draft(clean_db, rev2, spec_v2)
    with pytest.raises(PublishGateError, match="destructive"):
        publish_draft(clean_db, rev2)


def test_publish_allows_destructive_slot_drop_with_flag(clean_db):
    st = _str_type()
    id_slot = Slot(name="imdb_id", range=st, identifier=True, required=True)
    title = Slot(name="title", range=st)
    movie = OntologyClass(name="Movie", slots=[id_slot, title])
    ...
    rev2 = create_draft(clean_db)
    update_draft(clean_db, rev2, spec_v2)
    publish_draft(clean_db, rev2, allow_destructive=True)  # must not raise
    assert not _column_exists(clean_db, "movie", "title")
