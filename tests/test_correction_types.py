...
def test_split_writes_audit_entry(ct_db):
...
def test_split_partitions_bindings(ct_db):
    conn, movie, src_a, src_b, rev = ct_db
    apply_split(
        conn,
        cls=movie,
    ...
    assert not graph_store.canonical_id_exists(conn, cls=movie, canonical_id="tt_main")
    # Both new canonical ids should exist.
    assert graph_store.canonical_id_exists(conn, cls=movie, canonical_id="tt_split_1")
    assert graph_store.canonical_id_exists(conn, cls=movie, canonical_id="tt_split_2")


def test_split_contributions_routed_correctly(ct_db):
    conn, movie, src_a, src_b, rev = ct_db
    apply_split(
        conn,
        cls=movie,
    ...
    c1 = graph_store.get_canonical_contributions(conn, cls=movie, canonical_id="tt_split_1")
    c2 = graph_store.get_canonical_contributions(conn, cls=movie, canonical_id="tt_split_2")
    assert len(c1) == 1 and c1[0]["_source"] == "source_a"
    assert len(c2) == 1 and c2[0]["_source"] == "source_b"


def test_split_writes_lineage_event(ct_db):
    conn, movie, src_a, src_b, rev = ct_db
    apply_split(
        conn,
        cls=movie,
...
def test_split_returns_int_correction_id(ct_db):
    conn, movie, src_a, src_b, rev = ct_db
    result = apply_split(
        conn,
        cls=movie,
...
def test_add_creates_canonical_id(ct_db):
    conn, movie, src_a, src_b, rev = ct_db
    apply_add(
        conn,
        cls=movie,
    ...
    assert graph_store.canonical_id_exists(conn, cls=movie, canonical_id="tt_synthetic")


def test_add_source_is_user_corrections(ct_db):
    conn, movie, src_a, src_b, rev = ct_db
    apply_add(
        conn,
        cls=movie,
    ...
    contribs = graph_store.get_canonical_contributions(conn, cls=movie, canonical_id="tt_synthetic")
    assert len(contribs) == 1
    assert contribs[0]["_source"] == "_user_corrections"


def test_add_slot_values_stored(ct_db):
    conn, movie, src_a, src_b, rev = ct_db
    apply_add(
        conn,
        cls=movie,
    ...
    contribs = graph_store.get_canonical_contributions(conn, cls=movie, canonical_id="tt_syn2")
    assert contribs[0]["title"] == "Has Title"
    assert contribs[0]["year"] == 1985


def test_add_writes_audit_entry(ct_db):
    conn, movie, src_a, src_b, rev = ct_db
    cid = apply_add(
        conn,
        cls=movie,
...
def test_add_writes_lineage_event(ct_db):
    conn, movie, src_a, src_b, rev = ct_db
    apply_add(
        conn,
        cls=movie,
...
def test_tombstone_entity_excluded_from_default_reads(ct_db):
    conn, movie, src_a, src_b, rev = ct_db
    apply_tombstone(conn, cls=movie, canonical_id="tt_extra", spec_revision=rev)
    # Default reads (include_tombstoned=False) should not see it.
    assert not graph_store.canonical_id_exists(conn, cls=movie, canonical_id="tt_extra")
    rows = graph_store.list_rows(conn, cls=movie)
    assert all(r["_canonical_id"] != "tt_extra" for r in rows)


def test_tombstone_include_tombstoned_shows_entity(ct_db):
    conn, movie, src_a, src_b, rev = ct_db
    apply_tombstone(conn, cls=movie, canonical_id="tt_extra", spec_revision=rev)
    rows = graph_store.list_rows(conn, cls=movie, include_tombstoned=True)
    tombstoned = [r for r in rows if r["_canonical_id"] == "tt_extra"]
    assert len(tombstoned) >= 1


def test_tombstone_count_with_include_tombstoned(ct_db):
    conn, movie, src_a, src_b, rev = ct_db
    normal_count = graph_store.count_rows(conn, cls=movie)
    apply_tombstone(conn, cls=movie, canonical_id="tt_extra", spec_revision=rev)
    count_after = graph_store.count_rows(conn, cls=movie)
    count_with_tombstoned = graph_store.count_rows(conn, cls=movie, include_tombstoned=True)
    assert count_after == normal_count - 1
    assert count_with_tombstoned == normal_count


def test_tombstone_contributions_with_include_tombstoned(ct_db):
    conn, movie, src_a, src_b, rev = ct_db
    apply_tombstone(conn, cls=movie, canonical_id="tt_extra", spec_revision=rev)
...
def test_tombstone_writes_audit_entry(ct_db):
    conn, movie, src_a, src_b, rev = ct_db
    cid = apply_tombstone(
...
def test_tombstone_writes_lineage_event(ct_db):
    conn, movie, src_a, src_b, rev = ct_db
    apply_tombstone(conn, cls=movie, canonical_id="tt_extra", spec_revision=rev)
    lineage = graph_store.list_lineage(conn, class_name="Movie")
...
def test_reject_contribution_removes_source_from_reads(ct_db):
    conn, movie, src_a, src_b, rev = ct_db
    ...
    contribs = graph_store.get_canonical_contributions(conn, cls=movie, canonical_id="tt_main")
    sources = [c["_source"] for c in contribs]
    assert "source_b" not in sources
    assert "source_a" in sources


def test_reject_contribution_canonical_id_still_exists(ct_db):
    conn, movie, src_a, src_b, rev = ct_db
    apply_reject_contribution(
        conn, cls=movie, canonical_id="tt_main", source="source_b", spec_revision=rev,
    )
    # canonical_id still exists because source_a still contributes.
    assert graph_store.canonical_id_exists(conn, cls=movie, canonical_id="tt_main")


def test_reject_contribution_source_row_preserved(ct_db):
...
def test_reject_contribution_writes_audit_entry(ct_db):
    conn, movie, src_a, src_b, rev = ct_db
    cid = apply_reject_contribution(
...
def test_reject_contribution_no_lineage_event(ct_db):
    """RejectContribution is a single-source change; no lineage event is emitted."""
    conn, movie, src_a, src_b, rev = ct_db
    lineage_before = graph_store.list_lineage(conn, class_name="Movie")
    apply_reject_contribution(
        conn, cls=movie, canonical_id="tt_main", source="source_b", spec_revision=rev,
    )
    lineage_after = graph_store.list_lineage(conn, class_name="Movie")
    assert len(lineage_after) == len(lineage_before)


def test_reject_both_sources_leaves_no_current_binding(ct_db):
...
def test_split_requires_nonexistent_new_canonical_ids(ct_db):
    ...
    already_exists = graph_store.canonical_id_exists(conn, cls=movie, canonical_id="tt_extra")
    assert already_exists


def test_add_idempotency_fails_on_duplicate(ct_db):
    """insert_synthetic_row on duplicate (_source, _source_row_id) raises — no ON CONFLICT."""
    conn, movie, src_a, src_b, rev = ct_db
    apply_add(conn, cls=movie, new_canonical_id="tt_dup", values={"imdb_id": "tt_dup"}, spec_revision=rev)
    import psycopg
...
def test_tombstone_nonexistent_returns_zero_closed(ct_db):
    conn, movie, src_a, src_b, rev = ct_db
    from knot.db import graph_store as gs
    closed = gs.tombstone_canonical_id(
        conn, cls=movie, canonical_id="tt_nonexistent", spec_revision=rev,
    )
    assert closed == 0


def test_reject_contribution_nonexistent_pair_returns_false(ct_db):
    conn, movie, src_a, src_b, rev = ct_db
    result = graph_store.reject_contribution(
