...
Verifies that the rename-slot flow:
  ...
  - Rejects rename when new_name collides with an existing slot.
  - Rejects rename when old_name doesn't exist on the draft.
"""
...
def _make_spec(slot_name: str = "title") -> tuple[Spec, Slot]:
...
async def test_rename_slot_preserves_data(clean_db):
    ...
    await rename_slot(clean_db, rev2, "title", "name")
...
async def test_rename_slot_is_not_destructive(clean_db):
    ...
    rev2 = await create_draft(clean_db, parent_revision=rev1)
    await rename_slot(clean_db, rev2, "title", "display_title")
...
async def test_rename_required_slot_renames_check_constraint(clean_db):
    ...
    # Old constraint name should be gone.
...
async def test_rename_slot_collision(clean_db):
    ...
    with pytest.raises(CollisionError):
        await rename_slot(clean_db, rev1, "title", "year")


# ---------------------------------------------------------------------------
# Test: old_name not on draft raises EntityNotOnDraftError
# ---------------------------------------------------------------------------


async def test_rename_slot_not_found(clean_db):
    """Renaming a slot that doesn't exist raises EntityNotOnDraftError."""
    spec_v1, _ = _make_spec("title")
    rev1 = await create_draft(clean_db)
    await update_draft(clean_db, rev1, spec_v1)

    with pytest.raises(EntityNotOnDraftError):
        await rename_slot(clean_db, rev1, "nonexistent", "something")
